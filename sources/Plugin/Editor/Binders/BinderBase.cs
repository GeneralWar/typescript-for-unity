...
namespace General.Typescript
{
    internal abstract partial class BinderBase
    {
        ...
        internal BinderBase(string name, BinderBase parent = null)
        {
            mName = name;
            mParent = parent;
            mBinderName = mName + "Binder";
            if (null == mParent)
            {
                mFullname = mName;
            }
        ...
        internal bool GenerateBinder()
        {
            ...
            if (null != content && content.Length > 0)
            {
                ...
                string outputPath = Path.Combine(sBinderOutputPath, string.Join(Path.DirectorySeparatorChar.ToString(), nameParts.Take(nameParts.Length - 1)));
                ...
                try
                {
                    File.WriteAllBytes(Path.Combine(outputPath, string.Concat(mBinderName, ".cs")), content);
                }
                catch (Exception e)
                {
        ...
        internal bool GenerateLibrary()
        {
            byte[] content = this.generateLibrary();
            if (null != content && content.Length > 0)
            {
                string[] nameParts = mFullname.Split('.');
                string outputPath = Path.Combine(sLibraryOutputPath, string.Join(Path.DirectorySeparatorChar.ToString(), nameParts.Take(nameParts.Length - 1)));
                if (!Directory.Exists(outputPath))
                {
                    Utility.CreateDirectory(outputPath);
                }
		...
		internal string BracketAsNamespace(out int levels)
		{
			...
			else
			{
				...
				++levels;
				string indent = string.Join("", Enumerable.Repeat("\t", levels));
				content = string.Concat(parent, string.Format("\n{0}", indent), string.Format("declare namespace {0}\n{1}{{", mName, indent));
			}
			return content;
		}

        abstract protected byte[] generateLibrary();
    }
}
