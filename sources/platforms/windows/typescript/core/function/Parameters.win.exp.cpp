...
char* General_Typescript_Parameter_GetTypeName(void* environment, void* pointer, int index)
{
	...
	Local<Object> object = value.As<Object>();
	Local<Value> isType = object->Get(String::NewFromUtf8(isolate, UNITY_INSTANCE_IS_TYPE));
	if (isType->IsNullOrUndefined())
	{
		Local<Value> property = object->Get(String::NewFromUtf8(isolate, UNITY_INSTANCE_TYPE));
	...
	else
	{
		...
		return ReturnStringToCSharp(GetCustomType(object).c_str());
	}
}
	...
	if (value->IsArray())
	{
		...
		else if (TYPE_NUMBER == type)
		{
			...
			else if (TYPE_NUMBER_SBYTE == subType)
			{
				...
				memcpy(buffer, &length, sizeof(int));
				char* p = buffer + sizeof(int);
				for (int i = 0; i < length; ++i, p += sizeof(char))
				{
					Maybe<int> maybe = localArray->Get(i)->Int32Value(context);
			...
			else if (TYPE_NUMBER_INT16 == subType)
			{
				...
				memcpy(buffer, &length, sizeof(int));
				char* p = buffer + sizeof(int);
				for (int i = 0; i < length; ++i, p += sizeof(short))
				{
					Maybe<int> maybe = localArray->Get(i)->Int32Value(context);
			...
			else if (TYPE_NUMBER_UINT16 == subType)
			{
				...
				memcpy(buffer, &length, sizeof(int));
				char* p = buffer + sizeof(int);
				for (int i = 0; i < length; ++i, p += sizeof(unsigned short))
				{
					Maybe<int> maybe = localArray->Get(i)->Int32Value(context);
			...
			else if (TYPE_NUMBER_INT32 == subType)
			{
				...
				memcpy(buffer, &length, sizeof(int));
				char* p = buffer + sizeof(int);
				for (int i = 0; i < length; ++i, p += sizeof(int))
				{
					Maybe<int> maybe = localArray->Get(i)->Int32Value(context);
			...
			else if (TYPE_NUMBER_UINT32 == subType)
			{
				...
				memcpy(buffer, &length, sizeof(int));
				char* p = buffer + sizeof(int);
				for (int i = 0; i < length; ++i, p += sizeof(unsigned int))
				{
					Maybe<int> maybe = localArray->Get(i)->Int32Value(context);
			...
			else if (TYPE_NUMBER_INT64 == subType)
			{
				...
				memcpy(buffer, &length, sizeof(int));
				char* p = buffer + sizeof(int);
				for (int i = 0; i < length; ++i, p += sizeof(long int))
				{
			...
			else if (TYPE_NUMBER_UINT64 == subType)
			{
				...
				memcpy(buffer, &length, sizeof(int));
				char* p = buffer + sizeof(int);
				for (int i = 0; i < length; ++i, p += sizeof(unsigned long int))
				{
					Local<Value> value = localArray->Get(i);
					if (value->IsBigInt())
					{
			...
			else if (TYPE_NUMBER_SINGLE == subType)
			{
				...
				memcpy(buffer, &length, sizeof(int));
				char* p = buffer + sizeof(int);
				for (int i = 0; i < length; ++i, p += sizeof(float))
				{
			...
			else if (TYPE_NUMBER_SINGLE == subType)
			{
				...
				memcpy(buffer, &length, sizeof(int));
				char* p = buffer + sizeof(int);
				for (int i = 0; i < length; ++i, p += sizeof(double))
				{
					Maybe<double> maybe = localArray->Get(i)->NumberValue(context);
